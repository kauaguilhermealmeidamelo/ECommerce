<template>
  <header class="topo">
    <div class="topo__marca">
      <span class="topo__logo"
        >{{ nomeExibido.slice(0, -1)
        }}<span class="topo__logo-destaque">{{
          nomeExibido.slice(-1)
        }}</span></span
      >
    ...
    <div class="topo__acoes">
      <div class="topo__notificacoes">
        ...
        <div v-if="notifAberta" class="topo__dropdown">
          ...
          <div v-if="notificacoes.length === 0" class="topo__dropdown-vazio">
            Nenhuma novidade por aqui.
          </div>
          <div
            v-for="(n, i) in notificacoes"
            :key="i"
            class="topo__dropdown-item"
          >
            <span
              class="topo__dropdown-ponto"
              :class="`topo__dropdown-ponto--${n.cor || 'azul'}`"
            ></span>
            <div>
              <p>{{ n.texto }}</p>
              <span>{{ n.tempo }}</span>
...
// Suporta tanto 'name' quanto 'nome' vindo da API/Store
...
const nomeExibido = computed(() =>
  (nomeLoja.value.split(/\s+/)[0] || "Loja").toUpperCase(),
);
