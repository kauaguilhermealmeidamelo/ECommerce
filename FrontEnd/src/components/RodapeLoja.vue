    ...
    <div class="rodape__conteudo">
      <div class="rodape__grade">
        ...
        <div class="rodape__coluna">
          <h4>Loja</h4>
          <ul>
            <li><router-link :to="{ name: 'catalogo' }">Catálogo</router-link></li>
            <li><router-link :to="{ name: 'meus-pedidos' }">Meus pedidos</router-link></li>
            <li><router-link :to="{ name: 'carrinho' }">Carrinho</router-link></li>
          </ul>
        </div>
      ...
      <div class="rodape__base">
        ...
        <p class="rodape__credito">
          ...
          <a href="https://github.com/kauag" target="_blank" rel="noopener">Kauã G.</a>
        </p>
      </div>
    </div>
  </footer>
...
const temRedeSocial = computed(() =>
  !!(loja.value?.instagram_url || loja.value?.facebook_url || loja.value?.tiktok_url || loja.value?.whatsapp)
)
...
const enderecoCompleto = computed(() => {
  ...
  return [`${endereco}${numero ? ', ' + numero : ''}`, bairro, cidade && uf ? `${cidade}/${uf}` : null]
    .filter(Boolean)
    .join(' — ')
})
