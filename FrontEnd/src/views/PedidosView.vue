  ...
  <div class="pagina">
    <div class="pagina__cabecalho">
      ...
      <div class="pagina__acoes">
        <button v-if="aba === 'todos'" class="btn btn--secundario" :class="{ 'btn--ativo': filtroAtivo }" @click="abrirFiltro">
    ...
    <div class="abas">
      <button class="abas__item" :class="{ 'abas__item--ativa': aba === 'todos' }" @click="aba = 'todos'">Todos os Pedidos</button>
    ...
    <div v-if="aba === 'todos'" class="card">
      ...
      <template v-else>
        <div class="tabela__scroll pedidos-tabela--desktop">
          <table class="tabela">
            <thead><tr><th>Pedido</th><th>Data</th><th>Itens</th><th>Entrega</th><th>Valor</th><th>Status</th><th></th></tr></thead>
            <tbody>
              <tr v-for="pedido in pedidosFiltrados" :key="pedido.id">
                ...
                <td>{{ rotuloEntrega(pedido.metodo_entrega) }}</td>
        ...
        <div class="lista-mobile pedidos-tabela--mobile">
          <div v-for="pedido in pedidosFiltrados" :key="pedido.id" class="lista-mobile__item" @click="verDetalhe(pedido)">
            <div style="flex:1">
              <strong>#{{ pedido.id }}</strong>
              <p class="pedidos__meta">{{ formatarData(pedido.created_at) }} · {{ pedido.itens?.length ?? 0 }} item(ns)</p>
            </div>
    ...
    <div v-else class="lista-mobile" style="display:flex; flex-direction:column; gap:.9rem">
      ...
      <div v-else-if="pedidosEnvio.length === 0" class="card estado-vazio">Nenhum pedido aguardando envio no momento.</div>

      <div v-for="pedido in pedidosEnvio" :key="pedido.id" class="card envio-card">
        <div class="envio-card__topo">
          <strong>Pedido #{{ pedido.id }}</strong>
          <span class="badge badge--azul">{{ pedido.metodo_entrega === 'local' ? 'Entrega local' : 'Transportadora' }}</span>
        </div>
        ...
        <details class="envio-card__form">
          ...
          <div class="envio-card__campos">
            ...
            <button class="btn btn--primario btn--bloco" :disabled="enviandoId === pedido.id" @click="marcarEnviado(pedido)">
    ...
    <Modal :aberto="filtroModalAberto" titulo="Filtrar Pedidos" @fechar="filtroModalAberto = false">
      <div class="filtro-campo">
        <label>Buscar</label>
        <div class="busca"><span>🔍</span><input v-model="filtroPendente.busca" placeholder="Número do pedido..." /></div>
      </div>
      <div class="filtro-campo">
        <label>Status</label>
        <div class="filtro-chips">
          <button v-for="s in ['Todos', 'pendente', 'pago', 'enviado', 'concluido', 'cancelado']" :key="s" class="filtro-chip" :class="{ 'filtro-chip--ativo': filtroPendente.status === s }" @click="filtroPendente.status = s">{{ s === 'Todos' ? 'Todos' : s }}</button>
        </div>
      </div>
      <template #rodape>
    ...
    <!-- Detalhe do pedido -->
    <Modal :aberto="pedidoDetalhe !== null" :titulo="`Pedido #${pedidoDetalhe?.id ?? ''}`" @fechar="pedidoDetalhe = null">
      <template v-if="pedidoDetalhe">
        ...
        <div class="detalhe-linha"><span>Entrega</span><strong>{{ rotuloEntrega(pedidoDetalhe.metodo_entrega) }}</strong></div>
        <div class="detalhe-linha"><span>Status</span><StatusPedidoBadge :status="pedidoDetalhe.status" /></div>
      </template>
    </Modal>
...
<style scoped>
.abas { display: flex; gap: .4rem; background: #fff; padding: .3rem; border-radius: var(--radius-md); border: 1px solid var(--line); width: fit-content; }
.abas__item { padding: .5rem 1rem; border-radius: var(--radius-sm); border: none; background: transparent; font-size: .8rem; font-weight: 700; color: var(--ink-soft); display: flex; align-items: center; gap: .4rem; }
.abas__item--ativa { background: var(--blue-600); color: #fff; }
.abas__contador { background: rgba(255,255,255,.25); font-size: .65rem; padding: .1rem .4rem; border-radius: 999px; }
.abas__item:not(.abas__item--ativa) .abas__contador { background: var(--blue-50); color: var(--blue-600); }

.pedidos-tabela--mobile { display: flex; flex-direction: column; }
.pedidos-tabela--desktop { display: none; }
@media (min-width: 640px) { .pedidos-tabela--mobile { display: none; } .pedidos-tabela--desktop { display: block; } }
.pedidos__meta { font-size: .72rem; color: var(--ink-faint); margin: .1rem 0 0; }
...
.envio-card { padding: 1rem; }
.envio-card__topo { display: flex; justify-content: space-between; align-items: center; margin-bottom: .5rem; }
.envio-card__itens { font-size: .78rem; color: var(--ink-soft); margin: 0 0 .8rem; }
.envio-card__endereco { display: flex; flex-direction: column; gap: .1rem; background: #fafbfc; border-radius: var(--radius-sm); padding: .75rem; font-size: .82rem; margin-bottom: .8rem; }
.envio-card__acoes { margin-bottom: .5rem; }
.envio-card__form summary { cursor: pointer; color: var(--blue-600); font-weight: 700; font-size: .8rem; }
.envio-card__campos { display: flex; flex-direction: column; gap: .6rem; margin-top: .8rem; }

.detalhe-linha { display: flex; justify-content: space-between; align-items: center; padding: .5rem 0; border-bottom: 1px solid var(--line); font-size: .85rem; }
.detalhe-linha span { color: var(--ink-soft); }
