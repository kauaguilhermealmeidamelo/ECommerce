  ...
  <div class="home">
    <section class="hero">
      <div class="hero__conteudo">
        ...
        <p class="hero__subtitulo">
          Peças selecionadas com cuidado, prontas pra fazer parte do seu estilo.
        </p>
        <router-link :to="{ name: 'catalogo' }" class="btn btn--primario"
          >Ver catálogo</router-link
        >
      </div>
    </section>
    ...
    <section class="secao secao--achadinhos">
      <div class="secao__cabecalho">
        ...
        <router-link :to="{ name: 'catalogo' }" class="secao__ver-tudo"
          >Ver tudo →</router-link
        >
      </div>

      <div v-if="carregandoAchadinhos" class="estado">Carregando...</div>
      <div v-else-if="achadinhos.length === 0" class="estado">
        Nenhum produto novo por aqui ainda.
      </div>
      <div v-else class="grade-produtos">
        <ProductCard
          v-for="produto in achadinhos"
          :key="produto.id"
          :produto="produto"
...
.secao__cabecalho .secao__titulo {
  margin: 0;
}
.secao__ver-tudo {
  font-size: 0.82rem;
