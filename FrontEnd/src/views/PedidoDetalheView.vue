    ...
    <template v-else-if="pedido">
      ...
      <section class="card">
        ...
        <ol class="linha-tempo">
          <li v-for="etapa in etapasRastreio" :key="etapa.chave" class="linha-tempo__item"
            :class="{ 'linha-tempo__item--concluida': etapa.concluida, 'linha-tempo__item--atual': etapa.atual }">
      ...
      <section class="card">
        ...
        <ul class="itens">
          <li v-for="item in pedido.itens" :key="item.id" class="itens__linha">
            <span>{{ item.quantidade }}x {{ item.produto?.nome }} <template v-if="item.tamanho">— {{ item.tamanho
                }}</template></span>
...
import { usePedidoDetalheViewModel } from '@/viewmodels/usePedidoDetalheViewModel'
...
.linha-tempo__item--concluida,
.linha-tempo__item--atual {
  opacity: 1;
}
...
.linha-tempo__item--concluida .linha-tempo__ponto {
  background: var(--cor-primaria);
}

.linha-tempo__item strong {
  display: block;
  font-size: .88rem;
  color: var(--cor-texto);
}
...
.rastreio-codigo span {
  font-size: .76rem;
  color: var(--cor-texto-suave);
}

.rastreio-codigo strong {
  font-size: .95rem;
  color: var(--cor-texto);
  letter-spacing: .03em;
}
