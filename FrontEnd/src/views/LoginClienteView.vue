...
.autenticacao__divisor { display: flex; align-items: center; gap: .75rem; margin: 1.25rem 0; color: var(--cor-texto-suave); font-size: .8rem; }
.autenticacao__divisor::before, .autenticacao__divisor::after { content: ''; flex: 1; height: 1px; background: var(--cor-linha); }

.btn { border: none; border-radius: var(--raio-borda); padding: .8rem 1rem; font-weight: 700; font-size: .92rem; cursor: pointer; text-align: center; text-decoration: none; display: inline-flex; align-items: center; justify-content: center; gap: .5rem; }
...
.btn--google { background: var(--cor-superficie); color: var(--cor-texto); border: 1px solid var(--cor-linha); }
.btn--google:hover { background: var(--cor-fundo); }
.btn--bloco { width: 100%; }
</style>
