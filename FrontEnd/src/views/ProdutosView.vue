  ...
  <div class="pagina">
    <div class="pagina__cabecalho">
      ...
      <div class="pagina__acoes">
        <button class="btn btn--secundario" :class="{ 'btn--ativo': filtroAtivo }" @click="abrirFiltro">
          <v-icon icon="mdi-magnify" size="small" class="mr-1"></v-icon> Filtros <span v-if="filtroAtivo"
            style="color: var(--blue-600)">●</span>
        </button>
        <router-link :to="{ name: 'categorias' }" class="btn btn--secundario">
          <v-icon icon="mdi-shape-outline" size="small" class="mr-1"></v-icon> Categorias
        </router-link>
        <router-link :to="{ name: 'produto-novo' }" class="btn btn--primario">
    ...
    <div class="grade-mini-stats">
      <div class="card mini-stat">
        <div class="icon-box icon-box--verde"><v-icon icon="mdi-check-circle-outline"></v-icon></div>
        <div><strong>{{ contagem.ativos }}</strong><span>Ativos</span></div>
      </div>
      <div class="card mini-stat">
        <div class="icon-box icon-box--ambar"><v-icon icon="mdi-alert-outline"></v-icon></div>
        <div><strong>{{ contagem.estoqueBaixo }}</strong><span>Estoque Baixo</span></div>
      </div>
      <div class="card mini-stat">
        <div class="icon-box icon-box--vermelho"><v-icon icon="mdi-package-variant-remove"></v-icon></div>
              ...
              <tr v-for="produto in filtrados" :key="produto.id">
                <td>
                  <div class="produto-linha" @click="irParaEdicao(produto)">
                    <img v-if="capaDoProduto(produto)" :src="capaDoProduto(produto)" class="produto-linha__imagem"
                      alt="" />
                    <div v-else class="produto-linha__imagem produto-linha__imagem--vazia">
                ...
                <td><span class="badge" :class="classeStatus(produto).badge">{{ classeStatus(produto).rotulo }}</span>
                </td>
                <td>
                  <div class="acoes-linha">
                    <button class="btn btn--fantasma" @click="irParaEdicao(produto)" title="Editar">
                      <v-icon icon="mdi-pencil-outline" size="small"></v-icon>
                    </button>
                    <button class="btn btn--fantasma" @click="confirmarExclusao(produto)" title="Remover">
        ...
        <div class="lista-mobile produtos-tabela--mobile">
          <div v-for="produto in filtrados" :key="produto.id" class="lista-mobile__item" @click="irParaEdicao(produto)">
            ...
            <div style="text-align:right">
              ...
              <div><span class="badge" :class="classeStatus(produto).badge">{{ classeStatus(produto).rotulo }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <Modal :aberto="filtroModalAberto" titulo="Filtrar Produtos" @fechar="filtroModalAberto = false">
      <div class="filtro-campo">
        ...
        <div class="busca">
          ...
          <input v-model="filtroPendente.busca" placeholder="Nome do produto..." />
        </div>
      </div>
      <div class="filtro-campo">
        <label>Categoria</label>
        <div class="filtro-chips">
          <button v-for="c in categoriasDisponiveis" :key="c" class="filtro-chip"
            :class="{ 'filtro-chip--ativo': filtroPendente.categoria === c }" @click="filtroPendente.categoria = c">{{ c
            }}</button>
        </div>
      </div>
      <div class="filtro-campo">
        <label>Status</label>
        <div class="filtro-chips">
          <button v-for="s in ['Todos', 'Ativo', 'Estoque Baixo', 'Esgotado']" :key="s" class="filtro-chip"
            :class="{ 'filtro-chip--ativo': filtroPendente.status === s }" @click="filtroPendente.status = s">{{ s
            }}</button>
        </div>
      </div>
