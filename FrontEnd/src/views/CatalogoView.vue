  ...
  <div class="pagina">
    ...
    <section v-else-if="busca">
      ...
      <div v-else class="grade-produtos">
        <ProductCard v-for="produto in produtosBusca" :key="produto.id" :produto="produto" @adicionar="adicionarAoCarrinho" />
      </div>
    </section>
    ...
    <section v-else-if="categoriaSelecionada">
      ...
      <div v-else class="grade-produtos">
        <ProductCard v-for="produto in produtosCategoriaUnica" :key="produto.id" :produto="produto" @adicionar="adicionarAoCarrinho" />
      </div>
    </section>
    ...
    <template v-else>
      <section v-for="secao in secoesPorCategoria" :key="secao.categoria.id" class="secao-categoria">
        <div class="secao-categoria__cabecalho">
          ...
          <button v-if="secao.total > secao.produtos.length" class="secao-categoria__ver-tudo" @click="categoriaSelecionada = secao.categoria.id">
            Ver tudo →
          </button>
        </div>

        <div v-if="secao.carregando" class="estado">Carregando...</div>
        <div v-else-if="secao.produtos.length === 0" class="estado-vazio estado-vazio--compacto">Nenhum produto nessa categoria ainda.</div>
        <div v-else class="grade-produtos">
          <ProductCard v-for="produto in secao.produtos" :key="produto.id" :produto="produto" @adicionar="adicionarAoCarrinho" />
        </div>
      </section>
...
const categoriaSelecionada = ref(route.query.categoria ? Number(route.query.categoria) : null)
...
async function carregarCategoriaUnica(categoriaId) {
  try {
    const { data } = await apiLoja.get('/produtos', { params: { categoria_id: categoriaId, por_pagina: 60 } })
...
async function carregarBusca(termo) {
  try {
    ...
    produtosBusca.value = data.data.filter((p) => p.nome.toLowerCase().includes(termoBusca))
  } catch (e) {
    produtosBusca.value = []
  }
}
...
<style scoped>
.pagina { max-width: 1100px; margin: 0 auto; padding: 1.25rem 1.25rem 6rem; }
.pagina__titulo { font-size: 1.4rem; margin: 0 0 1rem; color: var(--cor-texto); }

.pagina__busca-ativa { display: flex; align-items: center; gap: .75rem; font-size: .88rem; color: var(--cor-texto-suave); margin-bottom: 1.25rem; }
.pagina__busca-ativa strong { color: var(--cor-texto); }
.pagina__busca-ativa button { border: none; background: none; color: var(--cor-primaria); font-weight: 600; cursor: pointer; font-size: .82rem; }

.chips { display: flex; gap: .5rem; overflow-x: auto; padding-bottom: .5rem; margin-bottom: 1.5rem; }
.chip {
  flex-shrink: 0; border: 1px solid var(--cor-linha); background: var(--cor-superficie);
  color: var(--cor-texto-suave); font-size: .82rem; font-weight: 600;
  padding: .55rem 1rem; border-radius: 999px; cursor: pointer; white-space: nowrap;
  transition: background .15s, color .15s, border-color .15s;
}
.chip:hover { border-color: var(--cor-primaria); color: var(--cor-primaria); }
.chip--ativo { background: var(--cor-primaria); border-color: var(--cor-primaria); color: #fff; }

.estado { text-align: center; padding: 2.5rem 1rem; color: var(--cor-texto-suave); font-size: .88rem; }
.estado-vazio { text-align: center; padding: 2.5rem 1rem; color: var(--cor-texto-suave); font-size: .85rem; }
.estado-vazio--compacto { padding: 1.5rem 1rem; }

.secao__contador { font-size: .8rem; color: var(--cor-texto-suave); margin: 0 0 1rem; }

.secao-categoria { margin-bottom: 2.5rem; }
.secao-categoria__cabecalho { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem; }
.secao-categoria__cabecalho h2 { font-family: var(--fonte-display); font-size: 1.1rem; color: var(--cor-texto); margin: 0; }
.secao-categoria__ver-tudo { border: none; background: none; color: var(--cor-primaria); font-weight: 600; font-size: .8rem; cursor: pointer; }
