      ...
      <ul class="itens">
        <li v-for="item in itens" :key="item.id" class="item">
          ...
          <div class="item__info">
            ...
            <span v-if="item.tamanho" class="item__tamanho"
              >Tamanho: {{ item.tamanho }}</span
            >
          ...
          <div class="item__acoes">
            ...
            <div class="quantidade">
              ...
              <button @click="alterarQuantidade(item, item.quantidade + 1)">
                +
              </button>
            </div>
          </div>
...
async function alterarQuantidade(item, novaQuantidade) {
  ...
  try {
    const { data } = await apiLoja.patch(`/carrinho/itens/${item.id}`, {
